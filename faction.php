...
if(!$faction = load_cache(18, intval($id)))
{
	...
	if ($row)
	{
		...
		if($row['team']!=0)
			$faction['group'] = $DB->selectCell('SELECT name_loc'.$_SESSION['locale'].' FROM ?_udwbase_factions WHERE factionID=?d LIMIT 1', $row['team']);
		// Альянс(1)/Орда(2)
		if($row['side']!=0)
		...
		$item_rows = $DB->select('
			SELECT ?#, entry
			FROM ?_item_template i, ?_udwbase_icons a
			WHERE
		...
		$creature_rows = $DB->select('
			SELECT ?#, entry
			FROM ?_creature_template, ?_udwbase_factiontemplate
			WHERE
				faction_A IN (SELECT factiontemplateID FROM ?_udwbase_factiontemplate WHERE factionID=?d)
