...
if(!$spell = load_cache(13, intval($id)))
{
	...
	if ($row)
	{
		...
		// Дальность
		$RangeRow = $DB->selectRow('SELECT rangeMin, rangeMax, name_loc'.$_SESSION['locale'].' from ?_udwbase_spellrange where rangeID=? limit 1', $row['rangeID']);
		$spell['range'] = '';
		...
		// Время каста
		$casttime = $DB->selectCell('SELECT base from ?_udwbase_spellcasttimes where id=? limit 1', $row['spellcasttimesID']);
		if ($casttime>0)
		...
		$duration = $DB->selectCell('SELECT durationBase FROM ?_udwbase_spellduration WHERE durationID=?d LIMIT 1', $row['durationID']);
		if ($duration > 0)
			$spell['duration'] = ($duration/1000).' '.$smarty->get_config_vars('seconds');
		else
			$spell['duration'] ='<span class="q0">n/a</span>';
		// Школа спелла
		$spell['school'] = $DB->selectCell('SELECT name_loc'.$_SESSION['locale'].' FROM ?_udwbase_resistances WHERE id=?d LIMIT 1', $row['resistancesID']);
		// Тип диспела
		if ($row['dispeltypeID'])
			$spell['dispel'] = $DB->selectCell('SELECT name_loc'.$_SESSION['locale'].' FROM ?_udwbase_spelldispeltype WHERE id=?d LIMIT 1', $row['dispeltypeID']);
		// Механика спелла
		if ($row['mechanicID'])
			$spell['mechanic'] = $DB->selectCell('SELECT name_loc'.$_SESSION['locale'].' FROM ?_udwbase_spellmechanic WHERE id=?d LIMIT 1', $row['mechanicID']);
		...
		for ($j=1;$j<=2;$j++)
		{
			if ($row['tool'.$j])
			{
				$spell['tools'][$i] = array();
				// Имя инструмента
				$tool_row = $DB->selectRow('SELECT ?#, `quality` FROM ?_item_template, ?_udwbase_icons WHERE entry=?d AND id=displayid LIMIT 1', $item_cols[0], $row['tool'.$j]);
			...
			if($row['effect'.$j.'id'] > 0)
			{
				...
				if ($row['effect'.$j.'MiscValue'])
				{
					switch ($row['effect'.$j.'id'])
					{
						...
						{
							$spell['effect'][$i]['name'] .= ' ('.$DB->selectCell('SELECT name FROM ?_udwbase_skill WHERE skillID=? LIMIT 1', $row['effect'.$j.'MiscValue']).')';
							break;
						}
						// ауры
						case 6:
				...
				if ($row['effect'.$j.'radius'])
					$spell['effect'][$i]['radius'] = $DB->selectCell("SELECT radiusbase from ?_udwbase_spellradius where radiusID=? limit 1", $row['effect'.$j.'radius']);
				...
				if ($row['effect'.$j.'triggerspell'] > 0)
				{
					...
					$spell['effect'][$i]['spell']['name'] = $DB->selectCell('SELECT spellname_loc'.$_SESSION['locale'].' FROM ?_udwbase_spell WHERE spellID=?d LIMIT 1', $spell['effect'][$i]['spell']['entry']);
