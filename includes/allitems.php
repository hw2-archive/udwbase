...
function render_item_tooltip(&$Row)
{
	...
	if($Row['Map'])
		$x .= '<br />'.$DB->selectCell('SELECT name_loc'.$_SESSION['locale'].' FROM ?_udwbase_zones WHERE mapid=?d LIMIT 1', $Row['Map']);;
	...
	$x_tmp = '';
	$row = $DB->selectRow('SELECT ?# FROM ?_udwbase_itemset WHERE (item1=?d or item2=?d or item3=?d or item4=?d or item5=?d or item6=?d or item7=?d or item8=?d or item9=?d or item10=?d) LIMIT 1', $itemset_col[1], $Row['entry'], $Row['entry'], $Row['entry'], $Row['entry'], $Row['entry'], $Row['entry'], $Row['entry'], $Row['entry'], $Row['entry'], $Row['entry']);
	if($row)
	{
		...
		if($row['skillID'])
		{
			$name = $DB->selectCell('SELECT name_loc'.$_SESSION['locale'].' FROM ?_udwbase_skill WHERE skillID=?d LIMIT 1', $row['skillID']);
