...
function coord_db2wow($mapid, $x, $y, $global)
{
	...
	$rows = $DB->select("SELECT * FROM ?_udwbase_zones WHERE (mapID=? and x_min<? and x_max>? and y_min<? and y_max>?)", $mapid, $x, $x, $y, $y);
	...
	if (count($rows)==0)
	{
		...
		$row = $DB->selectRow("SELECT * FROM ?_udwbase_zones WHERE (mapID=? and x_min=0 and x_max=0 and y_min=0 and y_max=0)", $mapid);
		if ($row) {
			$wow['zone'] = $row['areatableID'];
			$wow['name'] = $row['name_loc'.$_SESSION['locale']];
		} else {
...
{
	global $DB;
	$faction['name'] = $DB->selectCell('SELECT name_loc'.$_SESSION['locale'].' FROM ?_udwbase_factions WHERE factionID = ?d LIMIT 1', $id);
	$faction['entry'] = $id;
	return $faction;
}
