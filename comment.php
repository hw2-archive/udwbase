...
switch($_REQUEST["comment"]):
	case 'add':
		...
		$newid = $DB->query('INSERT
			INTO ?_udwbase_comments(`type`, `typeid`, `userid`, `commentbody`, `post_date`{, ?#})
			VALUES (?d, ?d, ?d, ?, NOW(){, ?d})',
		...
		if (empty($_POST['replyto']))
			$DB->query('UPDATE ?_udwbase_comments SET `replyto`=?d WHERE `id`=?d LIMIT 1', $newid, $newid);
	...
	case 'delete':
		...
		$DB->query('DELETE FROM ?_udwbase_comments WHERE `id`=?d {AND `userid`=?d} LIMIT 1',
			$_GET['id'],
	...
	case 'edit':
		...
		if (IsSet($_POST['body']))
			$DB->query('UPDATE ?_udwbase_comments SET `commentbody`=?, `edit_userid`=?, `edit_date`=NOW() WHERE `id`=?d {AND `userid`=?d} LIMIT 1',
	...
	case 'rate':
		...
		*/
		...
		if (IsSet($_GET['rating']) and !($DB->selectCell('SELECT `commentid` FROM ?_udwbase_comments_rates WHERE `userid`=?d AND `commentid`=?d LIMIT 1', $_SESSION['userid'], $_GET['id'])))
			$DB->query('INSERT INTO ?_udwbase_comments_rates(`commentid`, `userid`, `rate`) VALUES (?d, ?d, ?d)',
